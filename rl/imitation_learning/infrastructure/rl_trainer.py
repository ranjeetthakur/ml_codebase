...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(self, itr, load_initial_expertdata, collect_policy, batch_size):
        ...
        if itr==0:
            with open(load_initial_expertdata,'rb') as fl:
                expert_data = pickle.load(fl)
            loaded_paths = []
            for exp_d in expert_data:
                loaded_paths.append(Path(exp_d['observation'],exp_d['image_obs'],exp_d['action'],exp_d['reward'],exp_d['next_observation'],exp_d['terminal']))
            # print('Expert data: ',len(expert_data), expert_data[0].keys())
            return loaded_paths, 0, None
        else:
